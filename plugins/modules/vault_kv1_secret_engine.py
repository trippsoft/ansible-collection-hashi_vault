...
EXAMPLES = r"""
- name: Create a new KV version 1 secret engine
...
- name: Remove a KV version 1 secret engine
  trippsc2.hashi_vault.vault_kv1_secret_engine:
    url: https://vault:8201
    auth_method: userpass
    username: '{{ user }}'
...
def ensure_engine_absent(
    ...
    Args:
        ...
        previous_mount_is_kv1 (str): The backend type of the secret engine.
    ...
    if previous_backend_type is None or previous_backend_type != 'kv-v1':
        try:
            raise Exception(f"The secret engine at '{engine_mount_point}' has backend '{previous_backend_type}' that is not a KV version 1 secret engine")
...
def ensure_engine_present(
        ...
        replace_non_kv1_secret_engine: bool) -> dict | Exception:
    """
    ...
    Args:
        ...
        replace_non_kv1_secret_engine (bool): Whether to replace a non-KV version 1 secret engine.
    ...
    if previous_backend_type is None or previous_backend_type != 'kv-v1':
        if not replace_non_kv1_secret_engine:
            try:
                raise Exception(f"The secret engine at '{engine_mount_point}' has backend '{previous_backend_type}' that is not a KV version 1 secret engine")
            except Exception as e:
                module.fail_json(msg=to_native(e), exception=traceback.format_exc())
...
def run_module():
    ...
    replace_non_kv1_secret_engine: bool = module.params.get('replace_different_backend_type')
