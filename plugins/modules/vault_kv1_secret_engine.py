...
def ensure_engine_absent(
    ...
    Args:
        ...
        previous_mount_config (Optional[dict]): The configuration of the secret engine.
        previous_mount_is_kv1 (Optional[str]): The backend type of the secret engine.
    ...
    if previous_backend_type is None or previous_backend_type != 'kv-v1':
        module.handle_error(
            VaultModuleError(
                message=f"The secret engine at '{engine_mount_point}' has backend '{previous_backend_type}' that is not a KV version 1 secret engine"
            )
        )
...
def ensure_engine_present(
    ...
    engine_mount_point: str = module.params['engine_mount_point']
    replace_non_kv1_secret_engine: bool = module.params['replace_different_backend_type']
...
def run_module() -> None:
    ...
    module: VaultSecretEngineModule = VaultSecretEngineModule(backend_type='kv-v1')
