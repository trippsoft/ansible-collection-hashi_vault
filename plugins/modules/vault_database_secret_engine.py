...
def ensure_engine_absent(
    ...
    if previous_backend_type is None or previous_backend_type != 'database':
        try:
            raise Exception(f"The secret engine at '{engine_mount_point}' has backend '{previous_backend_type}' that is not a Database secret engine")
...
def ensure_engine_present(
        ...
        replace_non_database_secret_engine: bool) -> dict | Exception:
    """
...
def run_module():
    ...
    replace_non_database_secret_engine: bool = module.params.get('replace_different_backend_type')
    ...
    if state == 'present':
        result = ensure_engine_present(
            ...
            previous_backend_type,
            desired_mount_config,
            replace_non_database_secret_engine
        )
